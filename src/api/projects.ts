...
function toProject(p: ApiProject, latestScan?: ApiScan): Project {
  return {
    ...
    health: latestScan?.riskSummary?.overallHealth ?? 'UNKNOWN',
    scanStatus: (latestScan?.status as Project['scanStatus']) ?? 'COMPLETED',
    highRiskCount: latestScan?.riskSummary?.highCount ?? 0,
    criticalCount: latestScan?.riskSummary?.criticalCount ?? 0,
    eolCount: latestScan?.riskSummary?.eolCount ?? 0,
    buildTool: 'Maven',
    configId: p.id,
    scanFrequency: '',
  };
